...
exports.transferWithSwap = async function transferWithSwap(web3, privateKey, tokenAddr, to, amount, tokenAddrOut, uniFee, subtractFees, gasFactor, onlyEstimate) {
  ...
  let gas;
  if (subtractFees)
    gas = await lib.sendPrivateKey(web3, privateKey, erc20Contracts[tokenAddr].methods.exactInputSingle(
      [tokenAddr, tokenAddrOut, uniFee, to, ((new Date().getTime() * 0.001) + 3600) | 0, amount, amountOutMin, 0]
    ), swapRouterContract, undefined, true);
  ...
  return await lib.sendPrivateKey(web3, privateKey, async (gasPrice) => {
    let tryAmount;
    if (subtractFees) {
      tryAmount = (new web3.utils.BN(amount)).sub((new web3.utils.BN(gas)).mult(new web3.utils.BN(gasPrice)));
      if (tryAmount == '0' || tryAmount[0] == '-')
        return;
    } else
      tryAmount = amount;

    return erc20Contracts[tokenAddr].methods.exactInputSingle(
      [tokenAddr, tokenAddrOut, uniFee, to, ((new Date().getTime() * 0.001) + 3600) | 0, tryAmount, amountOutMin, 0]
    );
  }, swapRouterContract, gasFactor, onlyEstimate);
}